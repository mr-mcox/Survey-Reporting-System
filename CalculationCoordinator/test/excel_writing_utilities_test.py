...
class ExcelWritingUtilityTestCase(unittest.TestCase):
	...
	def setUp(self):
		...
		ws1.cell(row=1,column=1).value = "WB1Value"
		ws1.title = "WB1Sheet"
		wb1.save('wb1.xlsx')
	...
	def test_copy_sheet_between_wb(self):
		...
		ws2.cell(row=1,column=1).value = "WB2Value"
		ws2.title = "WB2Sheet"
		wb2.save('wb2.xlsx')
		...
		self.assertEqual(test_sheet.cell(row=1,column=1).value,'WB1Value')
		self.assertEqual(orig_sheet.cell(row=1,column=1).value,'WB2Value')
