...
class WriteExcelTestCase(unittest.TestCase):
	...
	def test_write_mappings(self):
		coordinator = CalculationCoordinator()
		coordinator.dimension_integer_mapping = {'values':['a','b','c'],'integers':['0','1','2']}
		coordinator.excel_dashboard_file = 'test_file.xlsx'
		coordinator.write_excel_dashboard_menus()
		...
		self.assertEqual(value_column_values,coordinator.dimension_integer_mapping['values'])
		self.assertEqual(integer_column_values,coordinator.dimension_integer_mapping['integers'])
