...
def step(context):
	...
	assert context.result.set_index(['question_id','region']).loc[(1,'Atlanta'),'aggregation_value'] == 0.5
...
def step(context):
    assert set(context.result.columns) == {'question_id','region','aggregation_value','result_type'}